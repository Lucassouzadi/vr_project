...
public class GizmoManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject leftRightGizmo;
    [SerializeField] private GameObject handStatesGizmo;
    [SerializeField] private GameObject manoClassGizmo;
    [SerializeField] private GameObject handSideGizmo;
    [SerializeField] private GameObject continuousGestureGizmo;
    [SerializeField] private GameObject triggerTextPrefab;
    [SerializeField] private GameObject flagHolderGizmo;
    [SerializeField] private GameObject smoothingSliderControler;
    [SerializeField] private Slider bpmSlider;
    [SerializeField] private Image centerCircle;
    [SerializeField] private TextMeshProUGUI bpmText;
    [SerializeField] private GameObject depthEstimationGizmo;
    private GameObject wristInformationGizmo;
    private GameObject fingerInformationGizmo;
    [SerializeField] private GameObject contourInformationGizmo;
    private WristInfoGizmo wristInfoGizmo;
    private FingerInfoGizmo fingerInfoGizmo;
    [SerializeField] private ContourGizmo contourGizmo;

    [SerializeField] private Text currentSmoothingValue;

    [SerializeField] private bool _showGestureAnalysis;

    [SerializeField] private bool _showPickDrop;
    [SerializeField] private bool _showGrabRelease;
    [SerializeField] private bool _showSwipes;
    ...
    [SerializeField] private bool _showWarnings;
    ...
    [SerializeField] private bool _showClickTriggerGesture;
    private bool _showGrabTriggerGesture;
    private bool _showReleaseTriggerGesture;
    [SerializeField] private bool _showSmoothingSlider;
    [SerializeField] private bool _showDepthEstimation;

    [SerializeField] private bool _showWristInfo;
    [SerializeField] private bool _showFingerInfo;
    [SerializeField] private bool _showContour;
    [SerializeField] private bool skeleton3d;
    [SerializeField] private bool gestures;
    ...
    private TextMeshProUGUI palmCenterValue;
    private TextMeshProUGUI boundingBoxValue;

    private TextMeshProUGUI screenPositionValue;
    private TextMeshProUGUI worldPositionValue;
    private TextMeshProUGUI cameraPositionValue;
    private TextMeshProUGUI whateverValue;
    private GameObject sphere;
    private String handState = "up";
    ...
    {
        get { return _showContinuousGestures; }

        set { _showContinuousGestures = value; }
    }

    public bool ShowManoClass
    {
        get { return _showManoClass; }

        set { _showManoClass = value; }
    }

    public bool ShowHandSide
    {
        get { return _showHandSide; }

        set { _showHandSide = value; }
    }

    public bool ShowHandStates
    {
        get { return _showHandStates; }

        set { _showHandStates = value; }
    }
    ...
    {
        get { return _showPickTriggerGesture; }

        set { _showPickTriggerGesture = value; }
    }

    public bool ShowDropTriggerGesture
    {
        get { return _showDropTriggerGesture; }

        set { _showDropTriggerGesture = value; }
    }

    public bool ShowSwipeHorizontalTriggerGesture
    {
        get { return _showSwipeHorizontalTriggerGesture; }

        set { _showSwipeHorizontalTriggerGesture = value; }
    }

    public bool ShowSwipeVerticalTriggerGesture
    {
        get { return _showSwipeVerticalTriggerGesture; }

        set { _showSwipeVerticalTriggerGesture = value; }
    }

    public bool ShowClickTriggerGesture
    {
        get { return _showClickTriggerGesture; }

        set { _showClickTriggerGesture = value; }
    }

    public bool ShowGrabTriggerGesture
    {
        get { return _showGrabTriggerGesture; }

        set { _showGrabTriggerGesture = value; }
    }

    public bool ShowReleaseTriggerGesture
    {
        get { return _showReleaseTriggerGesture; }

        set { _showReleaseTriggerGesture = value; }
    }
    ...
    {
        get { return _showDepthEstimation; }
        set { _showDepthEstimation = value; }
    }
    ...
    {
        get { return _showFingerInfo; }
        set { _showFingerInfo = value; }
    }

    public bool ShowContour
    {
        get { return _showContour; }
        set { _showContour = value; }
    }
    ...
    void Update()
    {
        ...
        DisplayPalmCenter(trackingInfo.palm_center);
        DisplayHandBoundingBox(trackingInfo.bounding_box);
        DisplayScreenPosition(trackingInfo.palm_center);
        DisplayWorldPosition(trackingInfo.palm_center, trackingInfo.depth_estimation);
        DisplayCameraPosition();
        CountBPMCycles(trackingInfo.depth_estimation);
        DisplayWhateverValue();
    ...
    private void CountBPMCycles(float depthEstimation)
    {

        float depthLowerBound = 0.86f;
        float depthUpperBound = 0.88f;
        bpmSlider.minValue = 40;
        bpmSlider.maxValue = 160;

        if (depthEstimation != 0.0f)
            currentDepth = depthEstimation;

        if (depthEstimation > depthUpperBound)
        {
            if (handState == "up")
            {
                ...
                lastBPM = currentBPM;
                currentBPM = 60.0f / (currentTimestamp - lastTimestamp);
            }
            handState = "down";
        }
        else if (depthEstimation < depthLowerBound)
        {
            handState = "up";
        }
        
        bpmSlider.value = Mathf.Lerp(lastBPM, currentBPM, 2.0f * (Time.time - currentTimestamp));
        bpmText.text = (int) currentBPM + " bpm";
    }
    
    void DisplayWhateverValue()
    {
        depthEstimationGizmo.SetActive(ShowDepthEstimation);

        if (!whateverValue)
        {
            whateverValue = depthEstimationGizmo.transform.Find("WhateverValue").gameObject
        ...
        {
            Vector3 joint = ManomotionManager.Instance.Hand_infos[0].hand_info.tracking_info.skeleton.joints[i];
            Vector3 orientation = ManomotionManager.Instance.Hand_infos[0].hand_info.tracking_info.skeleton.orientation_joints[i];
            
            jointsInfo += i + " - joint:" + joint.ToString("F2") + "orientation:" + orientation.ToString("F2") + "\n";
        }
        Debug.Log(jointsInfo);
        // whateverValue.text = jointsInfo;
        whateverValue.text = handState + "\n" + currentDepth + "\n";
        
        Vector3 palmCenter = ManomotionManager.Instance.Hand_infos[0].hand_info.tracking_info.palm_center;
        palmCenter = new Vector3(palmCenter.x * Screen.width, palmCenter.y * Screen.height, 0.0f);
        float magnitude = (centerCircle.transform.position - palmCenter).magnitude;
        // whateverValue.text = magnitude.ToString("F2");
        
        if (magnitude < 275.0f)
        {
            centerCircle.color = new Color(0.5f, 0.8f, 0.3f);
        }
        else
        {
            centerCircle.color = new Color(0.9f, 0.5f, 0.5f);
        }
    }
    ...
    void DisplayDepthEstimation(float depthEstimation)
    {
        ...
        {
            depthEstimationValue = depthEstimationGizmo.transform.Find("DepthValue").gameObject
                .GetComponent<TextMeshProUGUI>();
        }
    ...
    void DisplayPalmCenter(Vector3 palmCenter)
    {
        depthEstimationGizmo.SetActive(ShowDepthEstimation);

        if (!palmCenterValue)
        {
            palmCenterValue = depthEstimationGizmo.transform.Find("PalmCenter").gameObject
                .GetComponent<TextMeshProUGUI>();
        }

        if (ShowDepthEstimation)
        {
            palmCenterValue.text = "x:" + palmCenter.x.ToString("F2")
                                        + "\ny:" + palmCenter.y.ToString("F2")
                                        + "\nz" + palmCenter.z.ToString("F2");
        }
    }

    void DisplayHandBoundingBox(BoundingBox boundingBox)
    {
        depthEstimationGizmo.SetActive(ShowDepthEstimation);

        if (!boundingBoxValue)
        {
            boundingBoxValue = depthEstimationGizmo.transform.Find("BoundingBoxValue").gameObject
                .GetComponent<TextMeshProUGUI>();
        }

        if (ShowDepthEstimation)
        {
            boundingBoxValue.text = "Width:" + boundingBox.width.ToString("F2")
                                             + "\nHeight:" + boundingBox.height.ToString("F2")
                                             + "\nTopLeft:\n" + boundingBox.top_left.ToString("F2");
        }
    }

    void DisplayScreenPosition(Vector3 palmCenter)
    {
        depthEstimationGizmo.SetActive(ShowDepthEstimation);

        if (!screenPositionValue)
        {
            screenPositionValue = depthEstimationGizmo.transform.Find("ScreenPositionValue").gameObject
                .GetComponent<TextMeshProUGUI>();
        }

        if (ShowDepthEstimation)
        {
            screenPositionValue.text = "x:" + (palmCenter.x * Screen.width).ToString("F2")
                                            + "\ny:" + (palmCenter.y * Screen.height).ToString("F2");
        }
        
    }

    void DisplayWorldPosition(Vector3 palmCenter, float depthEstimation)
    {
        depthEstimationGizmo.SetActive(ShowDepthEstimation);

        if (!worldPositionValue)
        {
            worldPositionValue = depthEstimationGizmo.transform.Find("WorldPositionValue").gameObject
                .GetComponent<TextMeshProUGUI>();
        }

        if (ShowDepthEstimation)
        {
            depthEstimation -= 0.5f;
            depthEstimation *= 2.0f;

            Vector3 screenPosition = new Vector3(
                Screen.width * palmCenter.x,
                Screen.height * palmCenter.y,
                10.0f * depthEstimation);
            Vector3 handPosition = Camera.main.ScreenToWorldPoint(screenPosition);

            worldPositionValue.text = "x:" + handPosition.x.ToString("F2")
                                           + "\ny:" + handPosition.y.ToString("F2")
                                           + "\nz:" + handPosition.z.ToString("F2");
            sphere.transform.position = handPosition;
    ...
    void DisplayCameraPosition()
    {
        ...
        if (!cameraPositionValue)
        {
            cameraPositionValue = depthEstimationGizmo.transform.Find("CameraPositionValue").gameObject
                .GetComponent<TextMeshProUGUI>();
        }
        ...
        if (ShowDepthEstimation)
        {
            ...
            cameraPositionValue.text = "x:" + cam.transform.position.x.ToString("F2")
                                            + "\ny:" + cam.transform.position.y.ToString("F2")
                                            + "\nz:" + cam.transform.position.z.ToString("F2");
        }
    }
